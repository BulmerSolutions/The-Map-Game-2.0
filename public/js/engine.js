          ...
          if (i < stack.length) {
            ...
            switch (stack[i]) {
              case this.mapping.fill.type:
                this.map.bucket.floodFill(
                  stack[i + 1],
                  stack[i + 2],
                  [stack[i + 3], stack[i + 4], stack[i + 5], stack[i + 6]],
                  stack[i + 7],
                  false
                );
              ...
              case this.mapping.pen.type:
                this.map.drawDot(
                  ...
                  stack[i + 3],
                  stack[i + 4],
                  [stack[i + 5], stack[i + 6], stack[i + 7], stack[i + 8]],
                  false
                );
...
class Bucket {
  ...
  constructor(canvas, game) {
    ...
    this.bitmap = this.ctx.getImageData(
      0,
      0,
      canvas.width - 1,
      canvas.height - 1
  ...
  async floodFill(x, y, color, tolerance, emit) {
    if (!this.isRendering) {
      ...
      let queue = [];
      let targetColor = Array.prototype.slice.call(
        this.bitmap.data,
        start,
        start + this.RGBA
      );
      ...
      if (
        !(
          Math.abs(color[0] - targetColor[0]) <= tolerance &&
  ...
  colorEquals(node, color, tolerance) {
    ...
    // Colors match
    if (
      Math.abs(node[0] - color[0]) <= tolerance &&
      Math.abs(node[1] - color[1]) <= tolerance &&
      Math.abs(node[2] - color[2]) <= tolerance &&
      Math.abs(node[3] - color[3]) <= tolerance
    )
      return false;
    ...
    this.canvas.addEventListener("click", function (e) {
      ...
      switch (self.tool.selected) {
        case "text":
          ...
          break;
        case "fill":
          let tolerance = Number.parseInt(
            document.getElementById("tolerance-wb").value
          );
          self.bucket.floodFill(
            pos.x,
            pos.y,
            self.hexToRGBA(self.tool.color, 255),
            tolerance
          );
          break;
      }
  ...
  init() {
    ...
    this.canvas.addEventListener("click", function (e) {
      ...
      switch (self.tool.selected) {
        ...
        case "fill":
          let tolerance = Number.parseInt(
            document.getElementById("tolerance-map").value
          );
          let opacity = Number.parseInt(
            document.getElementById("opacity-map").value
          );
          self.bucket.floodFill(
            pos.x,
            pos.y,
            self.hexToRGBA(self.tool.color, Math.abs((opacity / 100) * 255)),
            tolerance,
            true
          );
          break;
