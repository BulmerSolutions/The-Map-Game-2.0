...
class Database {
  ...
  verifyTables() {
    ...
    this.db.query(
      "CREATE TABLE IF NOT EXISTS `users` (`user_id` INT(4) NOT NULL AUTO_INCREMENT, `username` VARCHAR(21) NULL, `email` VARCHAR(255) NOT NULL, `password` VARCHAR(255) NULL, `joined` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, `google_id` VARCHAR(45) NULL, `token` VARCHAR(255) NOT NULL, PRIMARY KEY(`user_id`), UNIQUE INDEX `user_id_UNIQUE`(`user_id` ASC));",
      (err, result) => {
        if (err) {
  ...
  if (seek < users.length) {
    if (users[seek].username === username || users[seek].email === username) {
      if (bcrypt.compareSync(password, users[seek].password)) {
        ...
        let date = new Date();
        console.log(
          users[seek].name_first +
            " " +
            users[seek].name_last +
            " (" +
            username +
            ") logged in at " +
            date
